	...
	*/
	render(){
	  ...
	  if(this.displayData[3]){
			...
			displayObjectProps.push(
				<TableCell key="ico">
					<Link to={{pathname: 'employees/edit', state: {address: this.props.address, dni: this.displayData[0], name: this.displayData[1]}}}>
						<IconButton aria-label="Edit"> 	
	        				<Edit />
	      				</IconButton>
					  </Link>
	      			<IconButton onClick={() => {
					  this.contracts[this.props.contract].methods.removeEmployee(this.props.address).send().then(()=>{
					  	})
					  }} 
					  aria-label="Delete">
	        			<Delete />
